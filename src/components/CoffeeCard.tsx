...
export default function CoffeeCard({ coffee }: { coffee: DomainCoffeeResponse }) {
	...
	return (
		<Link href={`/coffees/${coffee.id}`} className="block group">
			<Card className="h-full flex flex-col border-border/50 bg-card/80 backdrop-blur-sm transition-all hover:border-gold/30 hover:shadow-[0_0_20px_rgba(255,213,0,0.05)]">
				<CardHeader className="pb-1">
					<div className="flex items-start justify-between gap-1">
						<CardTitle className="text-base leading-snug text-snow group-hover:text-gold transition-colors">{displayName}</CardTitle>
						{coffee.id && (
					...
					{coffee.roaster_name && (
						<Link
							...
							className="flex items-center gap-1.5 text-sm font-medium text-dusty-rose hover:text-gold transition-colors"
						>
				...
				<CardContent className="flex-1 space-y-2 pt-0">
					{origin && <p className="text-sm font-medium text-grey-olive">{origin}</p>}
					...
					{coffee.tasting_notes && coffee.tasting_notes.length > 0 && (
						<p className="text-xs text-grey-olive leading-relaxed">{coffee.tasting_notes.join(', ')}</p>
					)}
					...
					<div className="flex items-center justify-between text-sm">
						<div className="flex items-baseline gap-2">
							{coffee.price_per_100g_min ? (
								...
								<span className="font-bold text-gold font-mono">${(coffee.price_cents / 100).toFixed(2)}</span>
							) : null}
						</div>
						{coffee.weight_grams ? <span className="text-grey-olive font-mono text-xs">{coffee.weight_grams}g</span> : null}
					</div>
					{coffee.similarity_score ? (
						<p className="text-xs text-dusty-rose font-mono">{Math.round(coffee.similarity_score * 100)}% match</p>
					) : null}
				</CardContent>
			</Card>
		</Link>
